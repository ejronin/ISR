...
The machine-readable contract above is part of release qualification. Human-readable doctrine controls meaning; CI protects the semantic invariant set from silent regression.
...
- **Possibility is not evidence.** An alternative explanation receives evidentiary weight only when facts in the record support it. The mere ability to imagine an innocent, accidental, unauthorized, mistaken, or delayed-knowledge scenario does not make the evidence balanced and does not justify `UNRESOLVED`.
- **Do not invent an innocent knowledge-acquisition path.** If an actor later corrects an earlier denial or explanation, assess whether the record actually shows how and when the actor learned the new facts. A hypothetical chain such as “perhaps headquarters did not know yet” has zero evidentiary weight unless supported by chronology, records, testimony, system evidence, or other accepted facts.
- **Later self-admission can falsify an earlier categorical denial.** When the same responsible actor or institution later acknowledges the same act it previously denied, the later acknowledgment is affirmative falsifying evidence against the earlier proposition. `NARRATIVE_SUBSTITUTION` or narrative evolution may describe the relationship between statements, but it must not replace adjudication of the earlier proposition.
- **Later purposeful rationale can falsify an earlier accident or no-intent explanation.** If the same act is first attributed to malfunction, accident, lack of intent, or unauthorized action and a later authoritative statement says the act was deliberately taken because of a specified reason, that later statement is affirmative counterevidence to the earlier explanation. Test any claimed distinction in event, scope, mixed causation, or later discovery against evidence rather than inventing it.
- **A good-faith correction requires an evidenced correction path.** A later statement may show that an earlier false statement was made in good faith when the record supports a credible knowledge-acquisition path—for example, missing release reporting followed by a documented investigation and correction. Without such evidence, do not supply that exculpatory pathway on the actor's behalf.
