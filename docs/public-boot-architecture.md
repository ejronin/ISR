...
7. It initializes `window.ATLAS_PUBLIC_STATE` and renders all 26 routes from the single validated in-memory model. The shared evidence resolver, source service, timeline, charts, and MapView consume that model; none owns a parallel evidence pipeline.
