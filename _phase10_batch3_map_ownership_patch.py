...
path = Path('tests/browser-public-parity-batch3.js')
text = path.read_text(encoding='utf-8')

old_shipping = '''    const shipping = await cdp.eval(`(() => {
...
new_shipping = '''    const shipping = await cdp.eval(`(() => {
...
old_shipping_assert = """    assert.deepEqual(shipping.routeIds.sort(), routeIds.slice().sort());"""
new_shipping_assert = """    assert.equal(shipping.routeOwnerCount, 1, 'exactly one Shipping MapView must own transport route controls');
...
old_economy = '''    const economy = await cdp.eval(`(() => {
      const map = document.querySelector('[data-component="MapView"]');
      const bounds = JSON.parse(map.dataset.mapBounds);
      return {
...
new_economy = '''    const economy = await cdp.eval(`(() => {
      const maps = [...document.querySelectorAll('[data-component="MapView"]')];
      const routeOwners = maps.filter(candidate => candidate.querySelector('button[data-route-id]'));
      const map = routeOwners[0];
      const controls = map ? [...map.querySelectorAll('button[data-route-id]')] : [];
...
old_economy_assert = """    assert.deepEqual(economy.mapRouteIds.sort(), routeIds.slice().sort());
    assert.deepEqual(economy.directoryRouteIds.sort(), routeIds.slice().sort());"""
new_economy_assert = """    assert.equal(economy.routeOwnerCount, 1, 'exactly one Economy MapView must own transport route controls');
...
for old, new, label in [
    (old_shipping, new_shipping, 'Shipping MapView ownership block'),
    (old_shipping_assert, new_shipping_assert, 'Shipping ownership assertions'),
    (old_economy, new_economy, 'Economy MapView ownership block'),
    (old_economy_assert, new_economy_assert, 'Economy ownership assertions'),
]:
