...
(function PublicRecordUIR2(){
  ...
  function applyFreshness(){
    const latest=window.ATLAS_CURRENT_UPDATE_20260825_LATE;
  ...
  function ensureOverviewIntro(){
    ...
    const routes=E('div','pr2-route-grid');routes.append(
    ...
    sec.append(routes);const key=E('div','pr2-status-key');
    [['pr2-confirmed','Confirmed'],['pr2-claimed','Claimed / not independently verified'],['pr2-disputed','Disputed or attribution unresolved'],['pr2-unresolved','Unknown / unresolved']].forEach(([c,t])=>key.append(E('span',c,t)));sec.append(key);panel.prepend(sec);
  }
  ...
  function ensureEvidenceDrawerLink(){
    ...
    let b=$('.pr2-copy-record',header);if(!b){b=E('button','pr2-copy-record','Copy record link');b.type='button';const close=$('button:not(.pr2-copy-record)',header);if(close)header.insertBefore(b,close);else header.append(b);b.onclick=()=>{const s=window.AtlasState?.get?.(),url=window.AtlasState?.url?.('https://ejronin.github.io/ISR/')||location.href;copyUrl(url,s?.selectedRecord?'Record link copied':'View link copied');};}
  ...
  function bind(){
    document.addEventListener('click',e=>{const route=e.target.closest('[data-pr2-view]');if(route){window.showAtlasPanel?.(route.dataset.pr2View);return;}if(e.target.closest('.primary-nav,.secondary-nav,.analysis-nav,#timeline,#sources,#endgame,.isr-evidence-drawer'))[0,100,300].forEach(ms=>setTimeout(refresh,ms));},true);
