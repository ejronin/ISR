...
(function IranMessagingShifts20260827R1(){
  ...
  function install(model){
    ...
    A(box,'p','callout','Read each row left-to-right: what Iran said → the outside acts that narrowed or changed that lane → what actually happened → what Iran said next. A walk-back can be observed from the sequence; why Tehran changed lanes is separately labeled as inference. This is not a propaganda blacklist.');
    const key=A(box,'div','ims-key');
    [['ASSERTIVE / LEGITIMIZING MESSAGE','What Tehran publicly establishes as strength or a red line.'],['OPTIONALITY-PRESERVING / NEGOTIATING MESSAGE','Language that preserves room to bargain without openly surrendering the earlier line.'],['OBSERVED REALITY','What the sourced sequence establishes independently of either narrative.'],['PRACTICAL RECONCILIATION','What both messages can mean together; motive remains unproven unless evidence establishes it.']].forEach(([h,p])=>{const k=A(key,'div','');A(k,'strong','',h);A(k,'span','',p);});
