...
(function EndgameUxPlainLanguageR1(){
  ...
  function overview(shell,strategic){
    ...
    if(ps[1])text(ps[1],'Tehran increasingly frames success around survival and avoiding capitulation. Atlas keeps that rhetoric separate from the original objectives and the current bargaining position.');
  ...
  function strategicPanel(shell){
    ...
    const paths=section(panel,['Iran’s three live paths · position test','Iran’s three live paths']);if(paths){paths.dataset.eg4Section='paths';text($('.eg3-section-head h3',paths),'Iran’s three live paths');text($('.eg3-section-head p',paths),'Three ways Tehran can still try to improve its position. Each is measured against Iran’s own stated goals and what it is actually doing.');const proof=$('.eg25-proof',paths);if(proof){text($('h4',proof),'What changed in Iran’s negotiating position');const steps=$$('.eg25-proof-step',proof);const copy=[['Stated position','Tehran said Washington had to stop coercion, restore the old bargain, or move first before Iran could return to a political process.'],['Observed move','Iran then validated Pakistani mediation and accepted a joint Oman process while the old MOU stayed dead and its earlier economic terms were not simply restored.'],['What that means','Iran moved before Washington restored what Tehran had demanded. In bargaining terms, that is a retreat from the prior position — not surrender of the Iranian state.']];steps.slice(0,3).forEach((x,i)=>{text($('strong',x),copy[i][0]);text($('p',x),copy[i][1]);});}text($('.eg25-precision',paths),'Here, “capitulation” means backing away from a previously stated negotiating position under pressure. It does not mean state surrender, disarmament, occupation, or that every Iranian goal failed.');}
  ...
  function mouPanel(shell){
    ...
    renameSection(p,['Who did NOT get what','What neither side got'],'notgot','What neither side got','This keeps later political claims from being mistaken for terms that were actually in the MOU.');
    const bars=renameSection(p,['Bargaining balance · preserved comparative bars','Who moved how far'],'bars','Who moved how far','These bars compare how far each side moved from its starting position. They are not a war score and they do not assign blame.');
    ...
    jumpBar(p,[['what','What it was'],['clauses','Clauses'],['bars','Who moved'],['timeline','How it failed'],['p5','Paragraph 5'],['now','Where it is now']]);
  }
