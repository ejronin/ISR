...
(function EndgameThreeAngles20260825R3(){
  const DATA='./data/endgame-current-20260825-r2.json?v=20260825-r3';
  ...
  function source(id){return live?.sources?.[id]||window.ISREndgamePublicViewR1?.model?.()?.sources?.[id]||null;}
  ...
  function makeSection(){
    ...
    const note=E('p','eg3-warning-text eg25-precision','Precision note: “capitulation” here means retreat from Iran’s previously stated negotiating position under external pressure. It does not mean regime surrender, disarmament, occupation, or proof that every Iranian objective has failed.');s.append(note);
    return s;
  }
  function install(){
