...
(function ISRFinalMermaidOilRoutesR1(){
  ...
  const graphState=new WeakMap(), renderedSvg=new WeakMap();
  const ROUTES='./data/oil-routes-r1.json?v=20260825-r1';
  let supplementalLayer=null, routeModel=null, mapWrapped=false, lastView='snapshot', relabelQueued=false;
  ...
  function fitNewSvg(host){const svg=svgFor(host);if(!svg||renderedSvg.get(host)===svg)return;renderedSvg.set(host,svg);requestAnimationFrame(()=>{if(svgFor(host)===svg)fit(host);});}
  function setupAudit(){
    const host=$('#egMermaidHost'),controls=$('.eg-graph-controls');if(!host||!controls||!svgFor(host))return;
    button(controls,'FIT',()=>fit(host));button(controls,'−',()=>zoom(host,-.10));button(controls,'+',()=>zoom(host,.10));pan(host);host.dataset.trueFitReady='1';fitNewSvg(host);
  }
  ...
  function buildSupplementalLayer(){
    ...
    window.ISROilRoutesR1={layer:supplementalLayer,model:routeModel,sync:syncRoutes,fit:fitOilRouteBounds};syncRoutes();
  }
  ...
  function syncRoutes(view=window.atlasActiveView||window.AtlasState?.get?.().activeView||lastView){lastView=view||lastView;if(!supplementalLayer||!window.atlasMap)return;shouldShow(lastView)?supplementalLayer.addTo(window.atlasMap):window.atlasMap.removeLayer(supplementalLayer);scheduleRelabel();}
  function setText(el,text){if(el&&el.textContent!==text)el.textContent=text;}
  function relabelRoutes(){
    const layerBtn=$('[data-layer-name="Trade / logistics routes"]');setText(layerBtn,'Oil Routes — SCHEMATIC');
    $$('.secondary-tab,.primary-tab,.tab,button').forEach(b=>{const t=(b.textContent||'').trim();if(/^China\s*\/\s*Arctic routes$/i.test(t)||/^China Arctic routes$/i.test(t)||/^Arctic routes$/i.test(t))setText(b,'Oil Routes');});
  }
  function scheduleRelabel(){if(relabelQueued)return;relabelQueued=true;queueMicrotask(()=>{relabelQueued=false;relabelRoutes();});}
  function fitOilRouteBounds(){
    if(!window.L||!window.atlasMap||!routeModel)return;
    const bounds=L.latLngBounds([]);
    (window.ATLAS_DATA?.arcticRoutes||[]).forEach(r=>(r.coords||[]).forEach(([lat,lon])=>bounds.extend([lat,lon])));
    (routeModel.routes||[]).forEach(r=>(r.coords||[]).forEach(([lat,lon])=>bounds.extend([lat,lon])));
  ...
  function wrapMap(){
    ...
    if(typeof original==='function'){const wrapped=function(viewId){const out=original.apply(this,arguments);lastView=viewId||lastView;queueMicrotask(()=>{syncRoutes(lastView);if(lastView==='arctic'&&shouldShow(lastView))fitOilRouteBounds();});return out;};wrapped._isrOilRoutesR1=true;window.configureAtlasMap=wrapped;}
    const b=$('[data-layer-name="Trade / logistics routes"]');b?.addEventListener('click',()=>setTimeout(()=>syncRoutes(),0));
    window.AtlasState?.subscribe?.(s=>{if(s?.activeView)lastView=s.activeView;syncRoutes(lastView);});scheduleRelabel();syncRoutes();
    if(lastView==='arctic'&&shouldShow(lastView))queueMicrotask(fitOilRouteBounds);
    const mo=new MutationObserver(scheduleRelabel);mo.observe(document.body,{childList:true,subtree:true});
  }
