...
(function SiteLegibilityR3(){
  ...
  const OFFICIAL_STATE_OUTLETS=new Set([
    'xinhua','anadolu agency','saudi press agency','irna','islamic republic news agency','press tv',
    'u.s. central command','us central command','centcom','air combat command','u.s. department of defense',
    'us department of defense','department of defense','white house','u.s. department of state','us department of state',
    'u.s. department of the treasury','us department of the treasury','iranian foreign ministry','iran ministry of foreign affairs',
    'oman foreign ministry','saudi ministry of defense','pakistan ministry of foreign affairs','turkish ministry of national defence'
  ]);
  ...
  function decorateActorLabels(){
    $$(ACTOR_ROOTS).forEach(root=>$$(ACTOR_NODES,root).forEach(exactActorNode));
  }
  ...
  function governmentLink(card){
    return $$('a[href]',card).some(a=>{try{const h=new URL(a.href,location.href).hostname.toLowerCase();return /(?:^|\.)(?:gov|mil)(?:\.[a-z]{2})?$/.test(h)||/\.gov\.(?:sa|ir|om|pk|tr|uk|ae|qa|bh|kw)$/.test(h);}catch(_){return false;}});
  }
  function markOfficialSources(){
    $$('#sources .isr-outlet-card').forEach(card=>{
      ...
      const name=(nameNode.textContent||'').trim(),profile=profiles.get(name),official=card.classList.contains('isr-gov-source')||profileOfficial(profile)||OFFICIAL_STATE_OUTLETS.has(norm(name))||governmentLink(card);
      if(!official)return;
      card.classList.add('isr-gov-source');
      if($('.isr-gov-source-star',head))return;
      const star=document.createElement('span');star.className='isr-gov-source-star';star.textContent='★';star.setAttribute('role','img');star.setAttribute('aria-label','Official government or state source');star.title='Official government / state source · provenance only';nameNode.insertAdjacentElement('afterend',star);
    });
  }

  function refresh(){window.ISRStatusIdentityR1?.refresh?.();window.ISRSourceBiasR1?.apply?.();decorateActorLabels();decoratePopups();markOfficialSources();}
  ...
  function bind(){
    ...
    document.addEventListener('click',e=>{if(e.target.closest('.primary-nav,.secondary-nav,#sources,#losses,.leaflet-popup,.layer-control'))[0,90,240].forEach(ms=>setTimeout(schedule,ms));},true);
    ['atlasstatechange','atlasdataready','atlascurrentready20260825late'].forEach(name=>window.addEventListener(name,schedule));
