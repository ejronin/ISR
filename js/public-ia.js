(function initAtlasPublicIA(globalObject, factory) {
  ...
  const EvidenceDrawer = Object.freeze({
    create(context, item, options) {
      ...
      append(details, 'summary', '', references.length || relatedRecords.length
        ? `Evidence & sources (${references.length + relatedRecords.length})`
        : 'Evidence & sources');
      ...
      if (item && item.event_id) {
        ...
        appendDefinition(facts, 'Knowledge time', temporal.knownBy);
  ...
  function addLossCard(host, context, record) {
    const card = addProvenanceCard(host, context, {
      ...
      title: `${formatQuantity(record)} · ${publicNarrative(record.item, 'Material item')}`, text: publicNarrative(record.note), technicalId: record.loss_id, item: record
    });
    ...
    addFactList(card, [['Service', publicNarrative(record.service, 'Unresolved')], ['Type', plainLabel(record.accounting_category, 'Unresolved')], ['Quantity status', record.quantity === null || record.quantity === undefined ? 'Unknown; not zero' : publicNarrative(record.quantity_qualifier, 'Exact quantity recorded')], ['Loss status', plainLabel(record.status, 'Unresolved')], ['Evidence confidence', plainLabel(record.confidence, 'Unresolved')], ['Date', readableDate(record.event_date)]]);
    if (record.side === 'CIVILIAN/COMMERCIAL') { const link = append(card, 'a', 'inline-route-link', context.route.key === 'hormuz.shipping' ? 'Open in the complete Losses record' : 'Open related Shipping & Trade context'); link.href = routeHref(context.route.key === 'hormuz.shipping' ? 'military.losses' : 'hormuz.shipping', { loss: record.loss_id }); }
    return card;
  }
  ...
  function CampaignsPage(context) {
    ...
    const boundary = addSection(frame.article, 'How an attack becomes a demonstrated effect'); const ladder = append(boundary, 'ol', 'evidence-ladder'); ['Launch', 'Defense / interception', 'Impact', 'Physical damage', 'Operational / mission effect', 'Strategic effect'].forEach(label => append(ladder, 'li', '', label)); append(boundary, 'p', 'section-note', 'Evidence at one stage does not establish the next. Any stage can remain unknown, unresolved or not demonstrated.');
  ...
  function InformationEnvironmentPage(context) {
    const frame = pageFrame(context, 'The Lie Ledger compares what actors said with what they did, what others did, what actually happened, and what the evidence now supports. Truth and deceptive intent remain separate: a false claim is not automatically a lie.'); const ledger = recordArray(modelData(context.model, 'gate3.lie_ledger')); const families = recordArray(modelData(context.model, 'gate3.narrative_families')); const chains = recordArray(modelData(context.model, 'gate3.information_chains')); const reliability = recordArray(modelData(context.model, 'gate3.source_reliability')); const deceptionLabels = ['No evidence of deception', 'Possible spin', 'Probable strategic misrepresentation', 'Strong evidence of knowing misrepresentation', 'Demonstrated lie']; const boundary = addSection(frame.article, 'How the ledger reaches a finding', 'content-section lie-ledger-boundary'); append(boundary, 'p', 'lead-copy', 'Truth and intent answer different questions. The truth finding tests the proposition against evidence and later behavior. The deception score asks what the originator could know when speaking. Repetition, political benefit, or a failed prediction is not by itself proof of a knowing falsehood.'); const flow = append(boundary, 'ol', 'claim-event-tree'); ['What they said', 'What they did', 'What others did', 'What actually happened', 'Current adjudication'].forEach(step => append(flow, 'li', '', step));
    const section = addSection(frame.article, 'Claim, contradiction and deception ledger'); append(section, 'p', 'section-note', `${ledger.length.toLocaleString()} proposition records are shown from the evidence state. Originators and amplifiers remain separate where the record supports that distinction.`); const controls = append(section, 'form', 'lie-ledger-controls'); controls.addEventListener('submit', event => event.preventDefault()); const searchLabel = append(controls, 'label', '', 'Search claims'); const search = append(searchLabel, 'input'); search.type = 'search'; search.placeholder = 'Search claims, actors or categories'; const truthLabel = append(controls, 'label', '', 'Truth finding'); const truth = append(truthLabel, 'select'); append(truth, 'option', '', 'All truth findings').value = ''; Array.from(new Set(ledger.map(record => record.truth_adjudication).filter(Boolean))).sort().forEach(value => { const option = append(truth, 'option', '', plainLabel(value)); option.value = value; }); const deceptionLabel = append(controls, 'label', '', 'Deception score'); const deception = append(deceptionLabel, 'select'); append(deception, 'option', '', 'All deception scores').value = ''; deceptionLabels.forEach((label, score) => { const option = append(deception, 'option', '', `${score} — ${label}`); option.value = String(score); }); const chainLabel = append(controls, 'label', '', 'Information chain'); const chain = append(chainLabel, 'select'); [['', 'All claim records'], ['linked', 'Linked to a chain'], ['unlinked', 'No chain recorded']].forEach(([value, label]) => { const option = append(chain, 'option', '', label); option.value = value; }); const count = append(controls, 'p', 'filter-result-count'); count.setAttribute('aria-live', 'polite'); const list = append(section, 'div', 'lie-ledger-list');
    const renderLedger = () => { const query = search.value.trim().toLowerCase(); const rows = ledger.filter(record => !query || JSON.stringify(record).toLowerCase().includes(query)).filter(record => !truth.value || record.truth_adjudication === truth.value).filter(record => chain.value !== 'linked' || record.chain_id || record.atlas_event_tree_link).filter(record => chain.value !== 'unlinked' || !record.chain_id && !record.atlas_event_tree_link).filter(record => deception.value === '' || String(record.deception_score) === deception.value); list.replaceChildren(); rows.forEach(record => { const details = append(list, 'details', 'lie-ledger-record'); details.dataset.claimId = record.claim_id; details.dataset.truthAdjudication = String(record.truth_adjudication || '').toLowerCase(); details.dataset.deceptionScore = String(record.deception_score); const summary = append(details, 'summary', 'lie-ledger-summary'); const heading = append(summary, 'span', 'lie-ledger-summary-copy'); append(heading, 'span', 'card-kicker', `${plainLabel(record.truth_adjudication, 'Unresolved')} · deception ${record.deception_score} of 4`); append(heading, 'strong', '', publicNarrative(record.proposition || record.claim, record.claim_id)); append(heading, 'small', '', `${publicNarrative(record.actor, 'Originator unresolved')} · ${plainLabel(record.claim_category, 'Other')}`); const body = append(details, 'article', 'record-card lie-ledger-detail'); appendActorIdentities(body, context, [record.actor || 'Originator unresolved']); addFactList(body, [['Claim', publicNarrative(record.claim)], ['Testable proposition', publicNarrative(record.proposition)], ['Originator or speaker', publicNarrative(record.speaker_institution || record.actor, 'Unresolved in the record')], ['Originator status', plainLabel(record.originator_status || record.claimant_type, 'Not separately resolved')], ['Claim category', plainLabel(record.claim_category, 'Other')], ['What happened', publicNarrative(record.adjudication_basis || record.inherited_assessment, 'The record does not supply a separate narrative beyond the current finding.')], ['What the actor did afterward', publicNarrative(record.subsequent_revision, 'No later correction or replacement is recorded for this proposition.')], ['External test / later evidence', asArray(record.later_evidence).length ? `${record.later_evidence.length.toLocaleString()} later evidence reference${record.later_evidence.length === 1 ? '' : 's'}; open Evidence & sources below.` : 'No separate external-test reference is recorded.'], ['Current yield', plainLabel(record.claim_test_status, 'Test not resolved')], ['Truth adjudication', plainLabel(record.truth_adjudication, 'Unresolved')], ['Deception score', `${record.deception_score} — ${deceptionLabels[Number(record.deception_score)] || 'Basis recorded below'}`], ['Deception basis', publicNarrative(record.deception_basis)], ['Narrative function', firstText(record.narrative_function) ? publicNarrative(record.narrative_function) : null], ['Knowledge access', plainLabel(record.knowledge_access, 'Not established')], ['Knowledge-access basis', publicNarrative(record.knowledge_access_note, 'No separate knowledge-access basis is recorded.')], ['Confidence', plainLabel(record.confidence, 'Unresolved')], ['What would change the rating', publicNarrative(record.what_would_change_rating)], ['Event time', publicNarrative(record.event_time, 'Not established')], ['Knowledge time', publicNarrative(record.knowledge_time, 'Not established')], ['Information-chain link', record.chain_id || record.atlas_event_tree_link || 'No chain link recorded']]); const publicLine = append(body, 'p', 'claim-public-sentence'); publicLine.textContent = `${publicNarrative(record.actor, 'The recorded actor')} claimed ${publicNarrative(record.proposition || record.claim)} Current finding: ${plainLabel(record.truth_adjudication, 'unresolved')}.`; const evidenceIds = [...new Set([...sourceIdsFrom(record), ...asArray(record.later_evidence)])]; if (evidenceIds.length) body.append(EvidenceDrawer.create(context, { source_ids: evidenceIds })); }); count.textContent = `${rows.length.toLocaleString()} of ${ledger.length.toLocaleString()} claim propositions shown`; if (!rows.length) append(list, 'p', 'empty-state', 'No claim propositions match these filters.'); };
  ...
  return Object.freeze({
    DEFAULT_ROUTE_KEY, PRIMARY_SECTIONS, ROUTE_DEFINITIONS, ROUTES, DATASET_LABELS, DISPLAY_TERMS, AFFILIATED_ACTORS, PERSON_PROFILES, PAGE_OWNERS, AppShell, PublicNavigation, EvidenceDrawer, ActorIdentity, EvidenceStatus, MapView, eventTemporalValues, eventEvidenceValues, displayTerm, publicNarrative, routeHref, parseRoute, routesForPrimary, modelData, recordArray, sortActorDirectory, validateRegistry, mount
  });
}));
