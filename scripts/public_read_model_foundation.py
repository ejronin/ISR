#!/usr/bin/env python3
"""Version-neutral public read-model foundation used by current compilers.

This module reproduces the accepted public foundation semantics that were
historically obtained by executing the legacy v1 public builder. It does not
import or execute that builder. The legacy generator path is read only as a
compatibility-lineage input so the established v1 public input-set fingerprint
remains stable while v2 is decoupled from the legacy compiler.
"""
...
LEGACY_SCHEMA_PATH = "schemas/public-current-state-v1.json"
LEGACY_GENERATOR_PATH = "scripts/build_public_current_state.py"
LEGACY_SCHEMA_VERSION = "1.0"
LEGACY_GENERATOR_VERSION = "1.3"
...
def build_compatibility_foundation(root: Path = ROOT) -> dict[str, Any]:
    """Build the accepted baseline public semantics without executing v1.

    The returned shape deliberately matches the legacy v1 state consumed by the
    v2 overlay. `LEGACY_GENERATOR_PATH` is hashed as lineage material only so the
    established v1 input-set fingerprint remains stable during decoupling.
    """
    ...
    entity_payloads = {
        ...
        "current.claims": {
            "schema_version": "1.0",
            "claims": [
                item["record"] for item in (canonical.get("entities") or {}).get("claims") or []
            ],
        },
        "current.material_losses": {
            ...
            "records": [
                item["record"]
                for item in (canonical.get("entities") or {}).get("material_losses") or []
            ],
        },
    ...
    counts["current_overlay_total"] = sum(
        item.get("contribution", 0)
        for item in input_packages
        if "OVERLAY" in str(item.get("role") or "")
    )
    counts.update({
    ...
    return {
        ...
        "authority_notice": (
            "Generated public view only. The sealed migration inputs and append-only "
            "accepted update packets remain authoritative."
        ),
        "generator": {
            "version": LEGACY_GENERATOR_VERSION,
