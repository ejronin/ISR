...
def validate_canonical_runtime_wiring() -> None:
    ...
    for token in (
        "window.registerAtlasMapMarker", "window.getAtlasMapMarker", "window.registerAtlasStrikeRecords",
        "window.refreshAtlasStrikeEffects", "function installStrikeMarker", "allMarkers[id]", "m.openPopup()",
    ...
    require("['Strike effects','Attacks / strikes']" in app, "public layer label must be Attacks / strikes")
    require("strikes:['Strike effects']" in app, "Campaigns & Strikes view must default the attack/strike layer on")
    ...
    for token in (
        "window.registerAtlasStrikeRecords", "result.eligible!==EXPECTED.strikes", "result.registered!==EXPECTED.strikes",
        "validateTimelineMapLinks", "window.getAtlasMapMarker", "map_linked_timeline_records", "registered_strike_markers",
        "temporal_contexts:[...new Set([...(row.temporal_contexts||[]),'strike'])]", "window.renderAtlasTimeline",
...
def validate_cutoffs() -> None:
    ...
    require("2026-08-20 15:59 ET" in app, "Aug. 20 outcome-synthesis re-adjudication cutoff must remain explicit")
    ...
    require("2026-08-26 16:30 ET" in recon, "current reconciliation cutoff must resolve to Aug. 26 16:30 ET")
...
def validate_mou_presentation_logic() -> None:
    ...
    for token in (
        "OBTAINED IN THE INTERIM DEAL", "DEFERRED TO FINAL NEGOTIATIONS — NOT YET WON OR LOST",
        "EXPLICITLY NOT INCLUDED / REJECTED", "IMPLEMENTATION AFTER SIGNATURE", "PROMISED BUT NEVER IMPLEMENTED",
        "LATER REVERSED", "Nuclear sequence · before → interim → deferred → movement → current",
        "Where the signed deal landed · analyst pole comparison", "Iran preferred endpoint", "U.S. preferred endpoint",
...
def validate_scoreboard_and_messaging() -> None:
    ...
    shim = text("js/iran-messaging-r1.js")
    messaging = text("js/iran-messaging-shifts-20260827-r1.js")
    navigation = text("js/navigation.js")
    ...
    for token in (
        "IRAN MESSAGING", "ASSERTIVE / LEGITIMIZING MESSAGE", "OPTIONALITY-PRESERVING / NEGOTIATING MESSAGE",
        "OBSERVED REALITY", "PRACTICAL RECONCILIATION", "HOW THE WALK-BACK CAN BE REFRAMED",
        "WHAT THE NEW POSITION LETS IRAN SAY", "WHAT CHANGED IN PRACTICE", "LOGICAL IMPLICATION · CONCESSION / WALK-BACK",
        "motive", "propaganda blacklist",
    ):
        require(token.lower() in messaging.lower(), f"Iran Messaging public structure missing: {token}")
    require("ISRIranMessagingShifts20260827R1" in shim and "iran-messaging-shifts-20260827-r1.js" in shim,
            "legacy Iran Messaging module must delegate to the canonical shift-series renderer")
    require("data-iran-messaging-r1" not in shim,
            "legacy Iran Messaging compatibility shim must not render the retired one-card panel")
    require("iran-messaging-r1.js" in navigation,
            "Iran Messaging compatibility entry point is not wired into public navigation")
...
def main() -> int:
    ...
    print(
        ...
        "cutoff separation, score hierarchy and canonical Iran Messaging shift-series contracts are enforced"
    )
    return 0
