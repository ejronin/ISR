...
def apply(state: dict[str, Any], root: Path, lie_module: Any, evidence_completion: Any) -> None:
    ...
    if update.get("evidence_cutoff") != state["release"].get("current_osint_cutoff"):
        raise ValueError("ROOK current-claim update cutoff does not match canonical current cutoff")
