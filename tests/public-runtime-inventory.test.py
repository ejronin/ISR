...
class PublicRuntimeInventoryTests(unittest.TestCase):
    def test_workflows_publish_and_browse_only_the_closed_artifact(self) -> None:
        ...
        self.assertIn("needs: [classify, qualify]", pages)
        self.assertIn("needs.classify.outputs.release_required == 'true'", pages)
        self.assertIn("QUALIFIED_SHA: ${{ needs.qualify.outputs.qualified_sha }}", pages)
