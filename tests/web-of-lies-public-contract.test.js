...
assert.match(iaSource, /Evidence behind this award and title/);
assert.match(iaSource, /What was bullshit/);
assert.match(iaSource, /What the evidence actually supports/);
assert.match(iaSource, /What this record establishes about their knowledge/);
assert.match(iaSource, /public availability of contrary evidence as proof/);
assert.match(iaSource, /amplificationObservationById/);
assert.match(iaSource, /wol-awardee-evidence/);
assert.doesNotMatch(iaSource, /publicly available contrary evidence[^\n]+(?:proves|establishes)[^\n]+(?:knew|knowledge)/i);
...
assert.match(shellSource, /\.wol-knowledge-boundary\s*\{/);
