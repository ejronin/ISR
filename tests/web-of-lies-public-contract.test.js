...
assert.match(iaSource, /Clear selection/);
assert.match(iaSource, /wol-hall-source-button/);
assert.match(iaSource, /selectionState = 'node'/);
assert.match(iaSource, /selectionState = 'connection'/);
...
assert.match(shellSource, /\.wol-node-detail\[data-selection-state="empty"\]\s*\{\s*display:\s*none/);
assert.match(shellSource, /\.wol-hall-source-button\s*\{/);
