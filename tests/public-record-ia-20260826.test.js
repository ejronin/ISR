...
assert.ok(!publicUi.includes("UNCONTESTED','NOT INDEPENDENTLY VERIFIED"), 'public UI must not equate uncontested with not independently verified');
...
]) assert.ok(presentation.includes(marker), `missing canonical presentation doctrine: ${marker}`);
assert.ok(!presentation.includes("UNVERIFIED: 'UNCONTESTED'"), 'UNVERIFIED must never be globally rewritten to UNCONTESTED');
assert.ok(!presentation.includes("NOT_INDEPENDENTLY_VERIFIED: 'UNCONTESTED'"), 'not independently verified must never be globally rewritten to UNCONTESTED');
...
console.log('public-record-ia-20260826: public hierarchy, evidence semantics, freshness, view defaults and analytic separation gates passed');
