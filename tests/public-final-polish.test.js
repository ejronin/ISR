...
assert(releaseCore.includes('reader_projection') && releaseCore.includes('reader_stylesheet'), 'single-pass core does not publish reader assets explicitly');
...
assert(appSource.includes("assetForRole(manifest, 'reader_projection')"), 'tracked public entrypoint does not authorize reader runtime');
